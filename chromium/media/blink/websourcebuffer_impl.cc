...
bool WebSourceBufferImpl::SetMode(WebSourceBuffer::AppendMode mode) {
  std::cout << "[WebSourceBufferImpl::SetMode]" << std::endl;
...
bool WebSourceBufferImpl::Append(const unsigned char* data,
  ...
  // std::cout << "[WebSourceBufferImpl::Append] " << old_offset.InSecondsF()
  //           << " " << append_window_start_.InSeconds() << " "
  //           << append_window_end_.InSeconds() << std::endl;
...
#ifdef JIMBERENABLED
  // std::cout << "[WebSourceBufferImpl::Append]" << id_ << std::endl;
  if (success) {
    blink::JCommandHandler::instance().sendAppendDataToSourceBuffer(
        windowId_, mediaSourceId_, id_, old_offset.InSecondsF(), data, length);
  }
#endif
...
void WebSourceBufferImpl::ResetParserState() {
  ...
  std::cout << "[WebSourceBufferImpl::ResetParserState]" << std::endl;
  blink::JCommandHandler::instance().sendResetSourceBufferParser(windowId_, mediaSourceId_,
                                                                 id_);
#endif
...
void WebSourceBufferImpl::ChangeType(const blink::WebString& content_type,
  ...
  std::cout << "[WebSourceBufferImpl::ChangeType]" << std::endl;
...
bool WebSourceBufferImpl::SetTimestampOffset(double offset) {
  // std::cout << "[WebSourceBufferImpl::SetTimestampOffset]" << std::endl;
  if (demuxer_->IsParsingMediaSegment(id_))
    return false;
...
void WebSourceBufferImpl::SetAppendWindowStart(double start) {
  ...
  // std::cout << "[WebSourceBufferImpl::SetAppendWindowStart] " << start
  //           << std::endl;
  // blink::JCommandHandler::instance().sendSetAppendWindowStartCommand(id_,
  //                                                                    start);
}
...
void WebSourceBufferImpl::SetAppendWindowEnd(double end) {
  ...
  // std::cout << "[WebSourceBufferImpl::SetAppendWindowEnd] " << end <<
  // std::endl;
  // blink::JCommandHandler::instance().sendSetAppendWindowEndCommand(id_, end);
}
