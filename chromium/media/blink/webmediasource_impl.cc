...
#ifdef JIMBERENABLED
  // std::cout << "[WebMediaSourceImpl::WebMediaSourceImpl]" << std::endl;
  // this->setId(base::GenerateGUID());
  // blink::JCommandHandler::instance().sendMediaSourceAction(
  //     this->getId(), jimber::MediaSourceAction::CREATE);
#endif
}
...
#ifdef JIMBERENABLED
    // std::cout << "[WebMediaSourceImpl::AddSourceBuffer]" << content_type.Utf8()
    //           << " " << codecs.Utf8() << std::endl;
    // blink::JCommandHandler::instance().sendAddSourceBuffer(
    //     this->getId(), id, content_type.Utf8(), codecs.Utf8());
#endif
  }
...
// #ifdef JIMBERENABLED
//   std::cout << "[WebMediaSourceImpl::MarkEndOfStream]" << std::endl;
//   blink::JCommandHandler::instance().sendMediaSourceAction(
//       this->getId(), jimber::MediaSourceAction::MARKENDOFSTREAM);
// #endif
