...
std::unique_ptr<WebContents> WebContents::CreateWithSessionStorage(
  ...
  std::cout << "[WebContents::CreateWithSessionStorage]" << std::endl;
...
std::unique_ptr<WebContentsImpl> WebContentsImpl::CreateWithOpener(
  ...
  // std::cout << "[WebContentsImpl::CreateWithOpener] params.jimber_window_id " << params.jimber_window_id << std::endl;
  // int n;
  // std::cin >> n;
  // if(n ==  3) {
  //   // raise(SIGSEGV);
  ...
  std::unique_ptr<WebContentsImpl> new_contents(
      new WebContentsImpl(params.browser_context));
  new_contents->SetOpenerForNewContents(opener, params.opener_suppressed);
  new_contents->m_jimberWindowId = params.jimber_window_id;
...
// JIMBER
void WebContentsImpl::setWindowId(int windowId) {
  m_jimberWindowId = windowId;
  GetRenderViewHost()->Send(
      new ViewMsg_SetWindowId(GetRenderViewHost()->GetRoutingID(), windowId));
}

void WebContentsImpl::selectMenuOptionByIndex(int index) {
  GetRenderViewHost()->Send(
      new ViewMsg_SelectMenuOptionByIndex(GetRenderViewHost()->GetRoutingID(), index));
}
// JIMBER END
...
std::unique_ptr<WebContents> WebContentsImpl::Clone() {
  ...
  // before.
  std::cout << "[WebContentsImpl::Clone]" << std::endl;
...
void WebContentsImpl::NotifyFullscreenChanged(bool will_cause_resize) {
  ...
  for (auto& observer : observers_)
    observer.DidToggleFullscreenModeForTab(!exitedFullscreen,
                                           will_cause_resize);
...
RenderFrameHostDelegate* WebContentsImpl::CreateNewWindow(
  ...
  DCHECK(opener);
  std::cout << "[WebContentsImpl::CreateNewWindow]" << std::endl;
...
WebContents* WebContentsImpl::OpenURL(const OpenURLParams& params) {
  ...
  DCHECK(params.Valid());
#endif
  std::cout << "[WebContentsImpl::OpenURL]" << std::endl;
  if (!delegate_) {
...
void WebContentsImpl::ViewSource(RenderFrameHostImpl* frame) {
  std::cout << "[WebContentsImpl::ViewSource]" << std::endl;
...
bool WebContentsImpl::CreateRenderViewForRenderManager(
  ...
  // std::cout << "[WebContentsImpl::CreateRenderViewForRenderManager]"
            // << std::endl;
  ...
  // Jimber
  // std::cout <<"render_view_host->Send " << m_jimberWindowId <<std::endl;
  render_view_host->Send(
      new ViewMsg_SetWindowId(render_view_host->GetRoutingID(), m_jimberWindowId));
  //
