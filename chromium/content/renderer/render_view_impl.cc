...
RenderViewImpl::RenderViewImpl(CompositorDependencies* compositor_deps,
  ...
  // std::cout << "[RenderViewImpl::ctor]" <<std::endl;
}
...
void RenderViewImpl::Initialize(
  ...
  // std::cout << "[RenderViewImpl::Initialize]" << std::endl;
  ...
  if (local_main_frame) {
    // std::cout << "[RenderViewImpl::Initialize] creating new mainframe"
    // << std::endl;

    main_render_frame_ = RenderFrameImpl::CreateMainFrame(
        this, compositor_deps, opener_frame, &params, std::move(show_callback));
  } else {
    // std::cout << "[RenderViewImpl::Initialize] CreateFrameProxy" <<
    // std::endl;
...
bool RenderViewImpl::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderViewImpl, message)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_SetWindowId, OnSetWindowId)
    IPC_MESSAGE_HANDLER(ViewMsg_SelectMenuOptionByIndex,
                        OnSelectMenuOptionByIndex)
    // jimber end
...
// jimber start
void RenderViewImpl::OnSetWindowId(int windowId) {
  int winId;

  auto mainFrame = GetWebView()->MainFrame();
  // std::cout << "[RenderViewImpl::OnSetWindowId] winid " << windowId<<
  // std::endl;

  // std::cout << "[RenderViewImpl::OnSetWindowId] ToWebLocalFrame "
  // << mainFrame->ToWebLocalFrame() << " " << std::endl;

  blink::Frame* coreFrame =
      WebFrame::ToCoreFrame(*reinterpret_cast<WebFrame*>(mainFrame));
  // std::cout << "OnSetWindowId " << coreFrame << std::endl;
  // std::cout << "[RenderViewImpl::OnSetWindowId] coreFrame " << coreFrame
  // << std::endl;
  coreFrame->windowId_ = windowId;
}

void RenderViewImpl::OnSelectMenuOptionByIndex(int index) {
  auto webview = GetWebView();
  blink::Element& owner = webview->GetPagePopupImpl()->PopupClient()->OwnerElement();
  blink::HTMLSelectElement& selectElement = reinterpret_cast<blink::HTMLSelectElement&>(owner);
  selectElement.SelectOptionByPopup(index);
