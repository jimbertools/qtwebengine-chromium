...
#ifdef JIMBERENABLED
  // if (GetExecutionContext()->ExecutingWindow()->GetFrame()->windowId_ != -1)
  // {
  JCommandHandler::instance().sendCreateNode(this);
  // }
#endif
}
...
#ifdef JIMBERENABLED
  if (!JTools::instance().isForbiddenDomAction(this, nullptr) &&
      Document::IsValidName(name.LocalName())) {
    if (!JTools::instance().isBlackListedAttribute(this->nodeName().Utf8(),
                                                   name.LocalName().Utf8(),
                                                   new_value.Utf8())) {
      std::string value =
          (this->nodeName() != "LINK" && this->nodeName() != "image" && this->nodeName() != "use") &&
                  name.LocalName() == "href"
              ? "javascript:void(0);"
              : new_value.Utf8();
      if (name.LocalName().Utf8() == "style" && !new_value.IsNull()) {
        value = JTools::instance().replaceAllURLs(&GetDocument(), value);
        JTools::instance().fixCSS(value);
      }
      JCommandHandler::instance().sendSetAttribute(
          this, name.LocalName().Utf8(), value, new_value.IsNull());
    }
  }
#endif
  if (new_value.IsNull()) {
...
void Element::AttributeChanged(const AttributeModificationParams& params) {
  ...
  if (!JTools::instance().isForbiddenDomAction(this, nullptr) &&
    ...
    if (!JTools::instance().isBlackListedAttribute(this->nodeName().Utf8(),
      ...
      if (name.LocalName().Utf8() == "style" && !params.new_value.IsNull()) {
        value = JTools::instance().replaceAllURLs(&GetDocument(), value);
        JTools::instance().fixCSS(value);
      }

      JCommandHandler::instance().sendSetAttribute(
          this, name.LocalName().Utf8(), value, params.new_value.IsNull());
    }
  }
#endif
}
...
#ifdef JIMBERENABLED
std::string& Element::getImageUuid() {
  return _imageUuid;
}
void Element::setImageUuid(const std::string& uuid) {
  _imageUuid = uuid;
}
#endif
...
ShadowRoot& Element::AttachShadowRootInternal(ShadowRootType type,
  ...
  // std::cout << "attachShadow to: " << intptr_t(this) << " " <<
  // this->nodeName()
  //           << "parent " << this->parentNode() << std::endl;
  if (!shadow_root.IsUserAgent() && this->nodeName() != "use") {
    JCommandHandler::instance().sendAttachShadow(this, &shadow_root);
  }
