...
Document::Document(const DocumentInit& initializer,
...
#ifdef JIMBERENABLED
  // if(!Url().IsNull()) {
  // int id1 = GetExecutionContext()->ExecutingWindow()->GetFrame()->windowId_;
  // std::cout << "[Document::Document] " << this->id() << " " << base_url_ << "
  // "
  // << id1 << std::endl;
  ...
  // std::cout << "[Document::Document] " << getpid() << " "<< frame_ <<
  // std::endl; std::cout << "[Document::Document] " <<
  // initializer.OwnerDocument() << std::endl;
  // } else {
  // std::cout << "[Document::Document] kgaat nie maken" << this->id()
  // <<std::endl
  // << this->location() ;
...
void Document::ApplyScrollRestorationLogic() {
  ...
  // UpdateStyleAndLayout to happen with subtree-visibility, which gets back
...
void Document::Shutdown() {
  // std::cout << "[Document::Shutdown]" << std::endl;
...
DocumentParser* Document::ImplicitOpen(
    ParserSynchronizationPolicy parser_sync_policy) {
  JCommandHandler::instance().sendClearDocument(this);
  RemoveChildren();
...
#ifdef JIMBERENABLED
  JCommandHandler::instance().sendSetScroll(target);
#endif
}
...
void Document::FinishedParsing() {
  ...
  if (IsHTMLDocument()) {
    if (!Url().IsNull()) {
      JCommandHandler::instance().sendDom(this);
    }
  }
