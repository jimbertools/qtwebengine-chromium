...
#include "third_party/blink/renderer/core/html/jimber/JCommandHandler.h"
#include "third_party/blink/renderer/core/html/jimber/JTools.h"
...
#ifdef JIMBERENABLED
    if (!JTools::instance().isForbiddenDomAction(this, new_child) &&
        // !(IsDocumentFragment() && !this->IsShadowRoot()) &&
        !(this->TreeRoot().IsShadowRoot() &&
          (DynamicTo<ShadowRoot>(this->TreeRoot())->GetType() ==
               ShadowRootType::kUserAgent ||
           DynamicTo<ShadowRoot>(this->TreeRoot())->host().nodeName() ==
               "use"))) {
      // for (const auto& target_node : targets) {
      if (new_child->IsTextNode() && this->nodeName() == "STYLE") {
        const Text* text = reinterpret_cast<const Text*>(new_child);
        std::string data = JTools::instance().replaceAllURLs(
            &GetDocument(), text->data().Utf8());
        JTools::instance().fixCSS(data);

        JCommandHandler::instance().sendSetCharacterData(new_child, data);
      }
      JCommandHandler::instance().sendInsertBefore(this, ref_child, new_child);
      // }
    }
#endif
      ...
      !(this->TreeRoot().IsShadowRoot() &&
        (DynamicTo<ShadowRoot>(this->TreeRoot())->GetType() ==
         ...
         DynamicTo<ShadowRoot>(this->TreeRoot())->host().nodeName() ==
             "use"))) {
    if (new_child->IsTextNode() && this->nodeName() == "STYLE") {
      Text* text = DynamicTo<Text>(new_child);
      std::string data = JTools::instance().replaceAllURLs(&GetDocument(),
                                                           text->data().Utf8());
      JTools::instance().fixCSS(data);

      JCommandHandler::instance().sendSetCharacterData(new_child, data);
    }
    JCommandHandler::instance().sendInsertBefore(this, &next_child, new_child);
  }
#endif
...
bool replaceChildOngoing = false;
Node* ContainerNode::ReplaceChild(Node* new_child,
  ...
  NodeVector post_insertion_notification_targets;
  {
    ...
    if (!JTools::instance().isForbiddenDomAction(this, new_child) &&
      ...
      JCommandHandler::instance().sendReplaceChild(old_child, new_child);
      // }
    }
...
void ContainerNode::WillRemoveChild(Node& child) {
  ...
  if (GetDocument() != child.GetDocument()) {
    // |child| was moved to another document by the DOM mutation event
    // handler.
    return;
  }
...
Node* ContainerNode::RemoveChild(Node* old_child,
  ...
  // FIXME: We should never really get PseudoElements in here, but editing
  // will sometimes attempt to remove them still. We should fix that and
  // enable this DCHECK.  DCHECK(!oldChild->isPseudoElement())
...
#ifdef JIMBERENABLED
  if (!replaceChildOngoing) {
    if (!JTools::instance().isForbiddenDomAction(this, &old_child) &&
        !(IsDocumentFragment() && !this->IsShadowRoot()) &&
        !(this->TreeRoot().IsShadowRoot() &&
          (DynamicTo<ShadowRoot>(this->TreeRoot())->GetType() ==
               ShadowRootType::kUserAgent ||
           DynamicTo<ShadowRoot>(this->TreeRoot())->host().nodeName() ==
               "use"))) {
      JCommandHandler::instance().sendRemoveChild(&old_child);
    }
  }
...
// This differs from other remove functions because it forcibly removes all
// the children, regardless of read-only status or event exceptions, e.g.
...
Node* ContainerNode::AppendChild(Node* new_child,
  ...
  NodeVector post_insertion_notification_targets;
  {
    ...
    if (!JTools::instance().isForbiddenDomAction(this, new_child) &&
      ...
      JCommandHandler::instance().sendAppendChild(this, new_child);
      // }
    }
#endif
...
void ContainerNode::ParserAppendChild(Node* new_child) {
  ...
  if (!JTools::instance().isForbiddenDomAction(this, new_child) &&
    ...
    // std::cout << "Really appending child " << this->id() << " " <<
    // new_child->id() << std::endl;
...
void ContainerNode::NotifyNodeInserted(Node& root,
  ...
  // std::cout << "ContainerNode::NotifyNodeInserted "  << std::endl;
#if DCHECK_IS_ON()
  DCHECK(!EventDispatchForbiddenScope::IsEventDispatchForbidden());
#endif
...
void ContainerNode::NotifyNodeRemoved(Node& root) {
  ...
  for (Node& node : NodeTraversal::InclusiveDescendantsOf(root)) {
    ...
    // of removal when they're not in the Document tree and not in a shadow
    // root since the virtual call to removedFrom is not needed.
...
void ContainerNode::AttachLayoutTree(AttachContext& context) {
  ...
  if (element && element->StyleRecalcBlockedByDisplayLock(
    ...
    // locking, none of its descendants should have the
...
void ContainerNode::RebuildChildrenLayoutTrees(
  ...
  // layout tree, and want to avoid a potentially n^2 loop to find the
  // insertion point while building the layout tree.  Having us start from the
  // last child and work our way back means in the common case, we'll find the
  // insertion point in O(1) time.  See crbug.com/288225
...
void ContainerNode::CheckForSiblingStyleChanges(SiblingCheckType change_type,
  ...
  // TODO(futhark@chromium.org): move this code into StyleEngine and collect
  // the various invalidation sets into a single InvalidationLists object and
...
// Takes an AtomicString in argument because it is common for elements to
// share the same name attribute.  Therefore, the NameNodeList factory
// function expects an AtomicString type.
...
// Takes an AtomicString in argument because it is common for elements to
// share the same set of class names.  Therefore, the ClassNodeList factory
// function expects an AtomicString type.
...
Element* ContainerNode::getElementById(const AtomicString& id) const {
  if (IsInTreeScope()) {
    // Fast path if we are in a tree scope: call getElementById() on tree
    // scope and check if the matching element is in our subtree.
