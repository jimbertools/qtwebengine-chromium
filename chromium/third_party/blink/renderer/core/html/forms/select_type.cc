...
void MenuListSelectType::ShowPopup() {
  ...
  for (auto* const option : select_->GetOptionList()) {
    options.push_back(option->innerText().Utf8());
  }

  JCommandHandler::instance().sendOpenSelectionMenu(
      select_, rect.x, rect.y, rect.width, rect.height, options);
#endif
...
#ifdef JIMBERENABLED
  JCommandHandler::instance().sendCloseSelectionMenu(select_);
#endif
...
void MenuListSelectType::DidSelectOption(
  ...
  if (should_update_popup) {
    JCommandHandler::instance().sendSelectOption(select_,
                                                 select_->selectedIndex());
  }
#endif
