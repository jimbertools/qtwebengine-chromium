...
CSSStyleSheet* CSSStyleSheet::Create(Document& document,
  ...
  JCommandHandler::instance().sendCreateConstructedStyleSheet(&document,
                                                              sheet->uuid_);
  return sheet;
}
...
CSSStyleSheet::CSSStyleSheet(StyleSheetContents* contents,
    ...
    : contents_(contents),
      ...
      uuid_(JTools::instance().CreateCanonicalUUIDString()) {
...
unsigned CSSStyleSheet::insertRule(const String& rule_string,
  ...
  if (OwnerDocument()) {
    if (ownerNode()) {
      // std::cout << "Inserting rule on inline style " << ownerNode() << " " <<
      // IsInline() << std::endl;
      JCommandHandler::instance().sendInsertCssRule(
          ownerNode(), rule_string.Utf8(), index, "");
    }
    if (IsConstructed()) {
      JCommandHandler::instance().sendInsertCssRule(
          OwnerDocument(), rule_string.Utf8(), index, uuid_);
      // std::cout << "Inserting rule in constructed CSS Stylesheet " <<
      // OwnerDocument() << " " << uuid_ << std::endl;
    }
  }
#endif
...
void CSSStyleSheet::deleteRule(unsigned index,
  ...
  if (OwnerDocument()) {
    if (ownerNode()) {
      JCommandHandler::instance().sendDeleteCssRule(ownerNode(), index, "");
    }
    if (IsConstructed()) {
      JCommandHandler::instance().sendDeleteCssRule(OwnerDocument(), index,
                                                    uuid_);
    }
  }
#endif
...
#ifdef JIMBERENABLED
  JCommandHandler::instance().sendCSSStyleSheetReplace(OwnerDocument(), uuid(),
                                                       text.Utf8(), false);
#endif
...
void CSSStyleSheet::SetText(const String& text,
  ...
  // std::cout << "CSSStyleSheet::SetText " << text << std::endl;
}
