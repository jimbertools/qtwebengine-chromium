...
// jimber
#ifdef JIMBERENABLED
  JCommandHandler::instance().sendAttachFontFaceToDocument(
      GetExecutionContext()->ExecutingWindow()->GetFrame()->GetDocument(),
      font_face->font_uuid_);
#endif
  //
  //
...
ScriptPromise FontFaceSet::load(ScriptState* script_state,
                                ...
                                const String& text) {
  // std::cout << "[FontFaceSet::load]" << std::endl;
  if (!InActiveContext())
