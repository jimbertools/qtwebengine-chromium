...
void AbstractPropertySetCSSStyleDeclaration::setCSSText(
    ...
    const String& text,
    ExceptionState&) {
  // std::cout << "AbstractPropertySetCSSStyleDeclaration::setCSSText " <<
  // ParentElement() << " " << text << std::endl;
...
#ifdef JIMBERENABLED
  std::string editableString = text.Utf8();
  JCommandHandler::instance().sendCssTextUpdate(ParentElement(),
                                                editableString);
#endif
...
// jimber
#ifdef JIMBERENABLED
  // std::cout << "Setproperty2 " << ParentElement() << " " <<
  // CSSProperty::Get(resolveCSSPropertyID(unresolved_property))
  //       .GetPropertyName() << " " << value << std::endl;
  JCommandHandler::instance().sendCssPropertyUpdate(
      ParentElement(),
      custom_property_name.IsNull()
          ? CSSProperty::Get(resolveCSSPropertyID(unresolved_property))
                .GetPropertyName()
          : custom_property_name.Utf8(),
      value.Utf8());
// jimber end
#endif
