...
#ifdef JIMBERENABLED
    if (resource) {
      if (resource->Url().ProtocolIs("blob")) {
        JCommandHandler::instance().sendImageWithBlobUrl(
            element_, resource->GetImage(), resource->Url().GetString().Utf8(),
            resource->GetResponse().MimeType().Utf8());
      }
    }
#endif
    // if (html_image_element->isConnected()) {
    //   std::string tmp =
    //       JTools::instance().generateQuerySelector(html_image_element);
    //   std::cout << "CONN Identified image at: " << tmp << std::endl;
    // } else {
    //   std::string tmp =
    //       JTools::instance().generateQuerySelector(html_image_element);
    //   std::cout << "UC Identified image at: " << tmp << std::endl;
    // }
  }
