...
String ImageResourceContent::getImageUuid() {
  return uuid;
}

void ImageResourceContent::generateRandomUuid() {
  uuid = WTF::CreateCanonicalUUIDString();
  // std::cout << "Generating random UUID for: " << uuid << std::endl;
}
...
void ImageResourceContent::AsyncLoadCompleted(const blink::Image* image) {
  ...
  std::cout << "Found async loaded images!" << std::endl;
...
ImageResourceContent::UpdateImageResult ImageResourceContent::UpdateImage(
  ...
  // std::cout << "Updating image: " << Url().GetString().Utf8() << std::endl;
