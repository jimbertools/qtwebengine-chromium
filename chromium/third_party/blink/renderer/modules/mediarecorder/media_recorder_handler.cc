...
bool MediaRecorderHandler::Initialize(MediaRecorder* recorder,
  ...
  // static int i = 0;
  // std::string filename = "/tmp/videos/hello" + std::to_string(i) + ".webm";
  ...
  if (!CanSupportMimeType(type, codecs)) {
    std::cout << "Unsupported " << type.Utf8() << ";codecs=" << codecs.Utf8()
              << std::endl;
...
void MediaRecorderHandler::HandleEncodedVideo(
    ...
    bool is_key_frame) {
#ifdef JIMBERENABLED
  // std::cout << "[MediaRecorderHandler::HandleEncodedVideo]" <<std::endl;
  // myfile << encoded_data;
  // JCommandHandler::instance().sendCanvasVideoData(
  //     recorder_->GetExecutionContext()
  //         ->ExecutingWindow()
  //         ->GetFrame()
  //         ->windowId_,
  //     jimberId_, encoded_data);
