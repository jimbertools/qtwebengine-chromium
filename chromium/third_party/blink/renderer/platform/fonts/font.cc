...
Font::Font(const FontDescription& font_description, FontSelector* font_selector)
  ...
  auto* window =
      DynamicTo<LocalDOMWindow>(font_selector->GetExecutionContext());

  Document* document = window->document();

  auto key = std::to_string(document->id()) +
             font_description.Family().Family().Utf8() +
             font_description.GetFontSelectionRequest().ToString().Utf8();
  
  auto primaryFont = PrimaryFont();
  auto fontData = primaryFont->PlatformData();
  SkString fontName;
  fontData.Typeface()->getFamilyName(&fontName);

  if (JTools::instance().isSentFont(key))
    return;

  JTools::instance().addSentFont(key);
  if (!document->Url().IsNull()) {
    if (!primaryFont->IsCustomFont()) {
      auto css_font_selector =
          reinterpret_cast<CSSFontSelector*>(font_selector);
      auto family = &font_description.Family();
      ...
      auto style = fontData.Typeface()->fontStyle();

      std::string family_name;
      std::string original_name;

      auto firstName = family->Family().Utf8();

      fc->matchFamilyName(
          fontName.c_str(),
          SkFontStyle(style.weight(), style.width(),
      ...
      if (path.empty()) {
        std::cout << "[Font::ctor] please make sure you have some fonts "
                     "installed. Ask @johnk apt-get install -y fonts-liberation"
                  << std::endl;
        exit(1);
      }

      if (firstName == "system-ui") {
        firstName = "j-system-ui";
      }

      if(firstName == "BlinkMacSystemFont") {
        firstName = "j-BlinkMacSystemFont";
      }

      if(firstName == "-apple-system") {
        firstName = "j-apple-system";
      }

      if(firstName == "ui-sans-serif") {
        firstName = "j-ui-sans-serif";
      }

      std::vector<std::string> components;
      path.GetComponents(&components);

      auto fileName = components.at(components.size() - 1);
      std::transform(firstName.begin(), firstName.end(), firstName.begin(),
      [](unsigned char c){ return std::tolower(c); });
      JCommandHandler::instance().sendCreateFontFace(
          document, firstName, fileName, result_style.slant(),
          result_style.weight(), result_style.width());
    }
  }
